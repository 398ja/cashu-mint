 ...
 *
 * <p>Archiving is how a keyset is retired — cashu-mint-admin archives a mint's keysets when an
 * operator retires it. Before this guard existed the signing path resolved the private key by
 * whatever keyset id the client supplied, so a retired mint carried on issuing. See ADR-0004.
 */
class MintProtocolUtilArchivedKeysetTest {
  ...
  void shouldRefuseToResolveKeyForArchivedKeyset() {
    ...
    try (MockedStatic<VaultClientFactory> factory = Mockito.mockStatic(VaultClientFactory.class)) {
      ...
      final CashuErrorException thrown =
          assertThrows(
