 ...
 *
 * <p>Drives {@code POST /v1/mint/bolt11} against a paid quote whose keyset the
 * vault reports as archived, and asserts what a wallet actually sees: a 4xx
 * carrying {@code keyset_inactive}, not a 500, and no tokens issued. The
 * complementary half — that an archived keyset still yields keys for redemption
 * through {@code getPrivateKey} — is pinned in
 * {@code MintProtocolUtilArchivedKeysetTest}.
 */
class ArchivedKeysetSigningIT extends AbstractMintDurableIT {
    ...
    @Autowired
    MintQuoteJpaRepository mintQuoteJpaRepository;

    @Autowired
    IssuanceRecordJpaRepository issuanceRecordJpaRepository;
    ...
    static void archiveTheKeyset() throws Exception {
        ...
        // archived keyset.
        Mockito.doThrow(new CashuErrorException(new ErrorResponse("keyset_inactive",
                        "Keyset " + TEST_KEYSET_ID + " is archived and no longer signs. "
                                + "Re-read /v1/keys and retry against an active keyset.").toJson()))
    ...
    void refusesToMintAgainstAnArchivedKeyset() {
        ...
        assertThat(response.getStatusCode().is4xxClientError())
                .as("an archived keyset is a client error, not a server fault — status=%s body=%s",
    ...
    private static Mint createMintWithKeys() {
        ...
        for (int amount : new int[] {1, 2, 4, 8}) {
            keys.put(BigInteger.valueOf(amount), PrivateKey.derivePublicKey(
