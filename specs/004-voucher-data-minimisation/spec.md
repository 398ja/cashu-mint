...
- Q: Hashing scheme for identity at rest? → A: HMAC-SHA-256 (key=salt, msg=value) — JDK-native via `javax.crypto.Mac`, no length-extension footgun, same perf as plain salted SHA-256, same output size.
- Q: Retention window default for identity columns? → A: 90 days (PT2160H). Operators can override via `cashu.mint.voucher.identity-retention`; default balances cross-quarter dispute resolution against minimisation.
- Q: Allow anonymous voucher purchases? → A: Yes — `customer_id` is nullable at creation; null means no identity stored at all. Funding gate stays correct because it keys on `(provider, provider_event_id)` via the webhook event, not on `customer_id`. Hash function MUST short-circuit on null input (no "hash of empty string" stored).
- Q: Grafana data source security model? → A: Separate read-only DB role with column-level `GRANT SELECT (col1, col2, ...)` that explicitly EXCLUDES identity columns (`customer_id`, `merchant_id`). PostgreSQL enforces the boundary at the DB layer — an ad-hoc Grafana query that selects an identity column fails with `permission denied`. Defence in depth on top of dashboard JSON review.
- Q: Backfill strategy for existing raw-npub rows on first boot post spec 004? → A: Online batched backfill — 1000-row chunks per transaction, idempotent (re-hashing a value yields the same hash so a crash-and-resume mid-batch is safe). Runs once at startup via Spring `@PostConstruct`; no table-level lock; concurrent writes to non-conflicting rows continue uninterrupted. Tracks progress via a `voucher_identity_backfill` log table so re-runs skip already-hashed batches.
...
- **FR-002**: Every identity-bearing column on `voucher_quote`, `customer_payment_funding`, `merchant_debit_funding`, `merchant_iou_funding`, and their `_aud` shadows MUST store `HMAC-SHA-256(key=mint_salt, msg=value)` rather than raw plaintext. Implementation MUST use `javax.crypto.Mac.getInstance("HmacSHA256")` (JDK-native, no third-party crypto dep). The salt MUST come from a configuration property (`cashu.mint.voucher.identity-salt`, no default — explicit operator action required), MUST be at least 256 bits of entropy, and MUST be the same salt across all tables. Output is a 64-char lowercase hex digest. **Null / empty inputs MUST short-circuit to a null column value** — no "hash of empty string" placeholder, since that would be enumerable and re-identifying. [US2 / Clarifications Q1 + Q3]
- **FR-003**: A scheduled purge job MUST nullify `customer_id` and `merchant_id` on `voucher_quote` rows in terminal states (`ISSUED` / `EXPIRED` / `FAILED`) older than `cashu.mint.voucher.identity-retention` (default `PT2160H` = 90 days; see Clarifications Q2). The purge MUST also nullify the same fields in every Envers revision for that row. Financial fields are retained per the **Retention Scope** section below — § A through § G enumerate every retained column and its purpose. [US3]
...
- **FR-011**: Migration of existing spec-003 rows: on first boot with FR-002 wired, the mint MUST backfill hashes for existing raw-npub rows via an **online batched** job (1000 rows per transaction, configurable via `cashu.mint.voucher.identity-backfill-batch-size`). The backfill MUST be **idempotent** — re-hashing a value produces the same hash, so a crash-and-resume mid-batch is safe; a separate `voucher_identity_backfill_log(table_name, last_hashed_pk, completed_at)` ledger tracks progress so re-runs skip already-hashed ranges. The job MUST NOT take a table-level lock; row-level locking only. The job MUST log a structured event per batch + a single summary line on completion (`voucher_identity_backfill complete rows={n} duration_ms={ms}`). After backfill completes, no plaintext npub remains in any voucher table OR Envers shadow. [Edge case / Clarifications Q5]
...
- **FR-013**: A PostgreSQL data source MUST be added to the `cashu-mint-observability` Grafana provisioning (`docker/grafana/provisioning/datasources/`) so dashboards can query the durable state directly. The credentials MUST come from environment variables. A dedicated read-only DB role (e.g. `cashu_mint_grafana_ro`) MUST be created by a Flyway migration; the role MUST be granted `SELECT` at **column level**, with **identity columns (`customer_id`, `merchant_id`) explicitly excluded** from the grant. An ad-hoc Grafana query that selects an identity column MUST fail with `permission denied` at the PostgreSQL layer — defence in depth on top of dashboard JSON review. The role MUST NOT have any DDL or write privileges. [US6 / Clarifications Q4]
...
- **FR-019**: Anonymous voucher purchases MUST be supported. When a quote is created without a `customer_id` (header / payload absent or explicit null), the durable `voucher_quote.customer_id` MUST be null and the funding-row resolution MUST still succeed via the `(provider, provider_event_id)` webhook anchor. Subsequent retention purge is a no-op for the already-null column. Operator audit queries MUST distinguish "anonymous" (`customer_id IS NULL` AND `created_at` within retention) from "purged" (`customer_id IS NULL` AND `created_at` past retention) — see FR-010's purged-revision marker. [US2 / Clarifications Q3]
...
- **VoucherIdentityBackfillLog** — append-only `voucher_identity_backfill_log(table_name, last_hashed_pk, completed_at)` table populated by the FR-011 backfill job. Lets operators verify backfill completion (`WHERE completed_at IS NOT NULL` for every table) and resume after a crash (`WHERE completed_at IS NULL ORDER BY last_hashed_pk` resumes from the last successful chunk).
...
- **SC-011**: Anonymous voucher purchases (FR-019) produce **zero identity storage** end-to-end. (Integration test: create a voucher quote with no `customer_id` header, complete the funding + issuance flow, query `voucher_quote.customer_id` and `customer_payment_funding.customer_id` for that quote — both MUST be null. No `{purged}` sentinel either, since the row was never identified.)
- **SC-012**: The Grafana DB role (FR-013) cannot read identity columns. (Integration test: connect as `cashu_mint_grafana_ro`, attempt `SELECT customer_id FROM voucher_quote LIMIT 1`, assert PostgreSQL returns `permission denied for column customer_id`. Same assertion for `merchant_id` and each funding child table.)
...
2. ~~Hashing scheme details — RESOLVED in Clarifications Q1: HMAC-SHA-256 via `javax.crypto.Mac`.~~
3. ~~Per-customer opt-out — RESOLVED in Clarifications Q3: anonymous purchases supported (FR-019). `customer_id` nullable at creation; null short-circuits hashing.~~
4. ~~Retention window default — RESOLVED in Clarifications Q2: 90 days (PT2160H), configurable via `cashu.mint.voucher.identity-retention`.~~
...
7. ~~Backfill strategy detail — RESOLVED in Clarifications Q5: online batched (1000 rows/txn, configurable), idempotent via re-hash determinism, progress tracked in `voucher_identity_backfill_log`, row-level locking only.~~
...
11. ~~Grafana data source security model — RESOLVED in Clarifications Q4: dedicated `cashu_mint_grafana_ro` role with column-level `GRANT SELECT` excluding identity columns. Provisioned via Flyway migration.~~
