...
- [ ] **T100** [P] [US1] Write `MintQuoteAmountBindingIT` in `cashu-mint-rest-it/src/test/java/.../it/MintQuoteAmountBindingIT.java`: scenarios under/over/exact + retry-with-same-outputs + retry-with-different-outputs (SC-001, FR-001/002/003). *(deferred — Testcontainers harness is in place (AbstractMintDurableIT, commit fed71dc) and the durable repositories are now threaded end-to-end (NUT04 → MintTokensTask → MintTask, commit f0ed646). The remaining blocker is that the REST mint endpoint calls `mintLoadService.load(mintId)` to load the keyset before MintTask runs; the IT needs a preloaded keyset matching the test `BlindedMessage` outputs plus a stubbed Gateway returning paid=true. Tracked as the next IT fixture follow-up. Unit-level coverage in `MintTaskAmountValidationTest` is comprehensive (8 cases).)*
- [ ] **T101** [P] [US1] Write `MintQuoteConcurrencyIT`: N concurrent identical mint requests against one quote; assert one `IssuanceRecord` row and identical signatures across all responses (SC-002). *(deferred — same fixture blocker as T100. Unit-level concurrency is covered by `IssuingConcurrencyTest`.)*
...
- [ ] **T300** [P] [US3] Write `MintQuoteIdempotentReplayIT`: same-outputs replay returns identical signatures; different-outputs replay returns `quote_already_issued`. *(deferred — same fixture blocker as T100. Unit-level coverage in `MintTaskAmountValidationTest#replay_with_same_outputs_returns_cached_signatures` and `IssuingConcurrencyTest#concurrent_issuing_is_replayed_once_state_reaches_issued`.)*
...
- [ ] **T900** [P] [X] Run `mvn -q verify -P integration-tests` end-to-end against a fresh Testcontainers Postgres; capture the full IT report and attach to the PR description. *(deferred — Testcontainers harness lives now (commits fed71dc, 7943fa1); a full reactor IT run is the next step after T100/T101/T300 land. Current spec-001 IT case count: 11 passing (T013 / T201 / T200+T202) under Testcontainers Postgres 16-alpine.)*
