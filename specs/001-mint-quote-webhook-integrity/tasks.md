...
- [X] **T901** [P] [X] Update Javadoc on `MintTask`, `MintQuoteTask`, `PaymentWebhookController`, `QuoteStatusUpdater`, `WebhookSignatureValidator` with NUT-04 / NUT-06 / NUT-19 / NUT-20 references. *(commit: pending; URLs currently point to `cashubtc/nuts/blob/main/{NN}.md`. Pinned-commit hashes per FR-014 / Constitution II remain a documented follow-up — choosing the canonical commit needs sign-off from the spec author.)*
- [X] **T902** [P] [X] Update `CLAUDE.md` `## Architecture` section. *(commit: pending; new `cashu-mint-jpa` module listed under main modules, spec 001 sub-section added under Architecture describing the three durable tables, the lifecycle state machine, and the `cashu.mint.jpa.enabled` feature flag.)*
- [X] **T903** [X] Audit advertised NUT-06 surface (SC-007). *(commit: pending; the `NutAdvertisementContractIT` Testcontainers form is deferred. Added `NutAdvertisementContractTest` (unit) that parses `mint.yaml` and asserts the advertised NUT set matches a documented test-backed allow list ({4, 5, 7, 8, 9, 10, 11, 12, 17}). Adding or removing an advertisement now fails this test until the inventory comment + EXPECTED constant are updated.)*
- [X] **T904** [X] Daily reconciliation queries embedded as class-level Javadoc on `MintQuoteJpaRepository`. *(commit: pending; two operator queries shipped — FR-001 invariant `SUM(mint_quote.amount WHERE ISSUED) == SUM(issuance_record.total_amount)`, plus a FR-005 invariant ensuring every `PAID`/`ISSUING`/`ISSUED` quote has a matching `webhook_event.outcome = accepted` row.)*
- [X] **T905** [X] Re-ran `LongArithmeticArchTest` against the now-merged code. *(commit: pending; gate still passes — see test output, 1 case green.)*
