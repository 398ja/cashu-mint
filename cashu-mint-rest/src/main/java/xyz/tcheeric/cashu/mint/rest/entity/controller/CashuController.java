...
public class CashuController<T extends Secret> {
    ...
    public ResponseEntity<PostMintQuoteResponse> quoteMint(@RequestBody PostMintQuoteRequest request,
                                                           ...
                                                           @org.springframework.web.bind.annotation.RequestParam(value = "unit", required = false) String unit) {
        var response = unit == null
                ? NUT04.quote(request.getAmount(), PaymentMethod.valueOf(method.toUpperCase()))
                : NUT04.quote(request.getAmount(), PaymentMethod.valueOf(method.toUpperCase()), unit);
        return ResponseEntity.ok(response);
    }
    ...
    public ResponseEntity<PostMintQuoteResponse> quoteMint(@PathVariable("method") String method,
                                                           ...
                                                           @org.springframework.web.bind.annotation.RequestParam(value = "unit", required = false) String unit) {
        PostMintQuoteResponse response = unit == null
                ? NUT04.quotePaymentStatus(quoteId, PaymentMethod.valueOf(method.toUpperCase()))
                : NUT04.quotePaymentStatus(quoteId, PaymentMethod.valueOf(method.toUpperCase()), unit);
    ...
    public ResponseEntity<PostMintResponse> mint(@RequestBody PostMintRequest<T> request,
                                                 ...
                                                 @org.springframework.web.bind.annotation.RequestParam(value = "unit", required = false) String unit) throws CashuErrorException {
        PostMintResponse response = NUT04.mint(UUID.fromString(mintId), request, PaymentMethod.valueOf(method.toUpperCase()), unit, signatureVaultService);
        return response == null ? ResponseEntity.notFound().build() : ResponseEntity.ok(response);
    }
    ...
    public ResponseEntity<PostMeltQuoteResponse> quoteMelt(@RequestBody PostMeltQuoteRequest request,
                                                           @PathVariable("method") String method,
                                                           @org.springframework.web.bind.annotation.RequestParam(value = "unit", required = false) String unit) {
        PostMeltQuoteResponse response = NUT05.quote(request, PaymentMethod.valueOf(method.toUpperCase()), unit, xyz.tcheeric.cashu.mint.proto.service.MintProtocolServiceFactory.getInstance());
        return ResponseEntity.ok(response);
    }
    ...
    public ResponseEntity<PostMeltQuoteResponse> quoteMelt(@PathVariable("method") String method,
                                                           @PathVariable("quote_id") String quoteId,
                                                           @org.springframework.web.bind.annotation.RequestParam(value = "unit", required = false) String unit) {
        PostMeltQuoteResponse response = NUT05.quotePaymentStatus(quoteId, PaymentMethod.valueOf(method.toUpperCase()), unit, xyz.tcheeric.cashu.mint.proto.service.MintProtocolServiceFactory.getInstance());
        return response == null ? ResponseEntity.notFound().build() : ResponseEntity.ok(response);
    }
    ...
    public ResponseEntity<PostMeltResponse> melt(@RequestBody PostMeltRequest<T> request,
                                                 @PathVariable("method") String method,
                                                 @PathVariable("mint_id") String mintId,
                                                 @org.springframework.web.bind.annotation.RequestParam(value = "unit", required = false) String unit) throws CashuErrorException {
        PostMeltResponse response = NUT05.melt(UUID.fromString(mintId), request, PaymentMethod.valueOf(method.toUpperCase()), unit);
        return response == null ? ResponseEntity.notFound().build() : ResponseEntity.ok(response);
    }
