...
- A running `cashu-mint-admin-rest` service (start
  ...
  with the `/v1/admin` routes enabled.
...
Point `MINT_ADMIN_API` at the host running the admin service—`cashu-mint-admin-rest`
exposes `/v1/admin` routes separately from the public mint API.
...
Send a `POST /v1/admin/lifecycle/mints` request with the operator details, display metadata, and initial configuration defined by [`CreateMintRequest`](../../cashu-mint-admin-rest/src/main/java/xyz/tcheeric/cashu/mint/admin/rest/dto/lifecycle/CreateMintRequest.java):

```bash
curl -X POST "$MINT_ADMIN_API/v1/admin/lifecycle/mints" \
...
To record a new configuration revision or change descriptive metadata, call `PUT /v1/admin/lifecycle/mints/{mintId}` with the schema from [`UpdateMintRequest`](../../cashu-mint-admin-rest/src/main/java/xyz/tcheeric/cashu/mint/admin/rest/dto/lifecycle/UpdateMintRequest.java):

```bash
curl -X PUT "$MINT_ADMIN_API/v1/admin/lifecycle/mints/mint-001" \
  -H "Content-Type: application/json" \
  -H "X-Admin-Token: $ADMIN_TOKEN" \
  -H "X-Admin-Roles: $ADMIN_ROLES" \
  -d '{
...
```bash
curl -X POST "$MINT_ADMIN_API/v1/admin/lifecycle/mints/mint-001/pause" \
  -H "Content-Type: application/json" \
  -H "X-Admin-Token: $ADMIN_TOKEN" \
  -H "X-Admin-Roles: $ADMIN_ROLES" \
  -d '{
...
curl -X POST "$MINT_ADMIN_API/v1/admin/lifecycle/mints/mint-001/resume" \
  -H "Content-Type: application/json" \
  -H "X-Admin-Token: $ADMIN_TOKEN" \
  -H "X-Admin-Roles: $ADMIN_ROLES" \
  -d '{
...
Retiring a mint follows the same pattern but targets `/v1/admin/lifecycle/mints/{mintId}/retire` and typically omits the correlation identifier:

```bash
curl -X POST "$MINT_ADMIN_API/v1/admin/lifecycle/mints/mint-001/retire" \
  -H "Content-Type: application/json" \
  -H "X-Admin-Token: $ADMIN_TOKEN" \
  -H "X-Admin-Roles: $ADMIN_ROLES" \
  -d '{
