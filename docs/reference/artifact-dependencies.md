...
```
cashu-lib-*     ┐
payment-adapter-*┼─ cashu-mint-protocol ─┬─ cashu-mint-rest ─┬─ cashu-mint-rest-it
...
- **Infrastructure (`cashu-vault-*`, `payment-adapter-*`).** Storage and payment adapters consumed by the protocol layer.
...
| `payment-adapter-common` | `cashu-lib-*` | Gateway SPI describing how the mint talks to external payment processors. |
| `payment-adapter-ln-phoenixd` | `payment-adapter-common` | Phoenixd Lightning implementation of the gateway SPI. |
| `payment-adapter-ln-dummy` | `payment-adapter-common` | Deterministic mock gateway used for testing and demo environments. |
| `cashu-voucher-*` | `cashu-lib-*` | Voucher domain/app/nostr components for the optional voucher profile. |
| `cashu-mint-protocol` | `cashu-lib-*`, `cashu-vault-*`, `payment-adapter-*`, `cashu-voucher-*` | Implements Cashu NUT workflows and exposes task/service APIs reused by REST and admin adapters. |
...
- `${payment-adapter.version}` → `payment-adapter-common`, `payment-adapter-ln-phoenixd`, `payment-adapter-ln-dummy`, `payment-adapter-rest`.
...
- Add a new gateway by implementing `payment-adapter-common` interfaces and mapping it via `gateway.<method>[.<unit>]`.
