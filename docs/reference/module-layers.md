...
- **Database access helpers.** The `DefaultMintLoadService` reads keysets and related state from the vault database by delegating to `DBMintVault`, keeping storage-specific concerns outside the controllers (see [`DefaultMintLoadService.java`](../../cashu-mint-protocol/src/main/java/xyz/tcheeric/cashu/mint/proto/service/DefaultMintLoadService.java)). The REST module does not use an application-local database.
